...
Settings -> Suno-промпты tab. Tuned for Suno v5.5 around a labeled-paragraph
Style-block (`Genre. Style. Mood. Instrumentation. Structure. Mastering.`)
pushed close to the field's real ~1000-char cap, with heavier, denser lyrics
decoration - the format/density the user's own ChatGPT/Gemini producer-prompt
workflow used and validated as giving richer results than a short comma-list
of descriptors. This supersedes an earlier, terser 250-600-char keyword-list
version. Presets only differ in the order of the Style-block's label
sentences - that's the one point where the source guides genuinely disagree
(vocal-first vs. canonical genre-first); everything else is shared."""
...
Порядок предложений-меток: Vocal (пол + характер) → Genre (жанр + эпоха) → Mood \
→ Instrumentation → Structure → Mastering. Vocal и Genre — всегда первые две \
метки: модель сильнее взвешивает начало промпта, а для этой аудитории главный \
риск — не жанр (он широкий и стабильный), а вокал (пол, тембр, «живость»)."""

_STYLE_ORDER_GENRE_FIRST = """\
Порядок предложений-меток — канонический: Genre → Mood → Instrumentation → \
Vocal → Structure → Mastering. Жанр всегда первый — он закладывает акустический \
фундамент, дальше настроение и инструментовка, вокал и структура ближе к концу."""

_BODY_TEMPLATE = """\
STYLE-BLOCK (только на английском). Пиши размеченным абзацем из шести подписанных \
предложений — "Label. Текст." — а не голым списком ключевых слов через запятую: \
`Genre. Style. Mood. Instrumentation. Structure. Mastering.` {style_order}
- Каждая метка — минимум одно полное предложение с конкретно названными \
элементами, а не расплывчатыми прилагательными: жанр+эпоха+темп/тональность, \
характер и пол вокала, 3-5 конкретно названных инструментов/текстур, набросок \
структуры с тайм-кодами (0:00 Intro → 0:15 Verse 1 → 0:45 Chorus → ... → Fade), \
mastering-хвост с loudness. Не экономь место — целься в 650-950 символов при \
потолке поля в 1000; Suno обрезает всё сверх лимита молча, без предупреждения, \
так что не переходи его, но заполняй доступное пространство содержательно, а не \
расплывчато — короткий и куцый блок здесь хуже, чем плотный.
...
- В Structure-метке явно перечисли тайм-коды и что происходит в каждом интервале \
(intro-hook, verse, chorus, bridge, final chorus, fade) — это самый надёжный \
способ управлять драматургией трека, не полагайся на то, что модель сама \
расставит акценты по остальным меткам.
- В Instrumentation-метке называй конкретные тембры и приёмы, а не общие \
категории: не "synths", а "bright supersaw pad, warm analog bass, DX7-style \
electric piano bells"; не "drums", а "punchy 909 kick, gated reverb snare".
...
плацебо (loudness в LUFS внутри Mastering-метки — не то же самое, это \
осмысленная единица, её оставляй). Вместо процентов — описательные термины: \
reverb-heavy / dry close-mic, bass-forward / deep sub-bass, compressed vocals \
/ tight mix.
...
Ретро-эффекты (gated reverb snare, analog synth arpeggios, orchestral hit \
stab, vocoder hook) — это палитра, а не деградация; заверши Mastering-метку \
современным хвостом (modern clean production, vocal-forward, wide stereo), \
чтобы вокал не тонул в плотном ретро-миксе.
- На трек — максимум два жанровых маркера в Genre-метке, доминирующий первым \
(eurodance + italo disco, pop rock + sentimental ballad); конфликтующие \
гибриды (lo-fi + modern clean production, raw production + polished) избегай.
- В Genre- или Structure-метке обязательно назови яркий, цепляющий hook или \
эффект интро (первые ~15 сек.), знакомый аудитории (мужчины и женщины 40-55 \
лет, ностальгия по хитам 80-90-х) — вставь конкретный тег вроде \
[vocoder spoken hook] или [orchestral hit stab] прямо на нужном тайм-коде \
внутри Structure-метки.
...
читается глазами.
- Разметка не опциональна, а обязательна: у каждого структурного блока должна \
быть хотя бы одна продакшн- или динамическая метка сразу после заголовка секции \
([Instrument: ...], [Build Up], [Full Band], [Half-Time], [Beat Enters], \
[Strings Swell], [Pre-Chorus Rise]) — без изменения слов оригинала. Не \
ограничивайся одной меткой на всю песню: чем плотнее прописана драматургия, тем \
предсказуемее результат.
- Между существующими смысловыми блоками (не внутри них) щедро вставляй \
интерлюдии — они не считаются новым блоком и не нарушают правило структуры \
выше: wordless-вокализы ((а-а-а...), (о-о-о...)), [Vocal_Chops «фраза»] на \
повторяющемся хуке припева, [Spoken_Shout_Female] или [Spoken_Shout_Male] в \
бридже, инструментальные вставки вроде [Instrument: Clean Guitar, Soft \
Strings], хоровые бэки [Choir_Backings]. Финальный припев усиливай метками \
[Final Chorus] + [Full Energy] и уводи трек в [Fade Out] на аутро.
- Любой петый текст, включая повторы и укороченные выжимки, должен быть основан \
строго на словах, уже присутствующих в оригинале — не сочиняй новые \
содержательные строки; декорации выше — это теги и wordless-вставки, а не новая \
лирика.
- Общая длина текста может немного меняться за счёт повтора припева или лёгкого \
сжатия куплета, но порядок и количество блоков (правило выше) остаются \
неизменными на протяжении всей песни — не только в первой половине.
...
Для дуэта/трио размечай роль перед каждой строкой, секции держи в пределах 8-12 \
строк — длиннее голоса начинают путаться местами.
- Надёжные вокальные теги (используй уверенно): male/female vocals, [Whispered], \
[Spoken Word], [Rap], [Belting], [Vocalise], [Harmony Vocals], [Ad-libs], raspy, \
breathy. Менее предсказуемые (используй экономно, закладывай 2-3 перегенерации): \
androgynous, [Call and Response], точный регистр вроде [Soprano]/[Alto] без \
подкрепления в Style, длинные многословные скобки-инструкции.
...
Style-block ≤1000 символов (целься в 650-950, ближе к потолку), Lyrics-markup — \
практический оптимум ~3500-4500 символов при лимите поля 5000 (плотная разметка \
из раздела выше добавляет длины — это нормально), сокращай только если реально \
упираешься в лимит.
"""
