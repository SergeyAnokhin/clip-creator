...
| `TimelineAudioTrack.jsx` | The timeline's audio row — decoded waveform `<canvas>` on the same px/ms scale. `scaleMode` prop (`'linear'`\|`'log'`\|`'sqrt'`, default `'linear'`, from `EditorStage.jsx`'s `waveformScale`/`EditorClipSettingsTab.jsx`'s picker) reshapes each peak (`scalePeak`) before it's drawn as a bar height - `log`/`sqrt` both raise quiet sections relative to loud ones, for spotting where a quiet passage ends and a loud one begins when picking a cut point. Only changes the bar-height formula, not the decode/`PEAK_BUCKETS` pipeline |
